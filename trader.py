...
def open_trade(record: dict) -> None:
    ...
    try:
        ...
        log.exception("%s | entry order failed", symbol)
        return
    ...
    log.info("%s | %s filled: id=%s price=%.4f qty=%.6f",
    ...
    try:
        ...
        log.info("%s | OCO placed: id=%s tp=%.4f sl=%.4f", symbol, oco_id, tp, sl)
    except Exception:
        log.exception("%s | OCO failed — attempting emergency market close", symbol)
        try:
            exchange.place_market_order(symbol, close_side, size)
            log.warning("%s | emergency close executed — position flat, no DB record", symbol)
            notify.send(
            ...
            log.critical(
                "%s | emergency close also failed — MANUAL INTERVENTION REQUIRED "
                "(open %s qty=%.6f entry=%.4f)",
...
def sync_open_trades() -> None:
    ...
    for trade in open_trades:
        ...
        if not oco_id:
            log.warning("%s | trade id=%d has no OCO — position is unmanaged", symbol, trade["id"])
            continue

        try:
            oco = exchange.client.get_order_list(orderListId=int(oco_id))
        except Exception:
            log.exception("%s | failed to fetch OCO %s", symbol, oco_id)
            continue
        ...
        for order_ref in oco.get("orders", []):
            try:
                ...
                log.exception("%s | failed to fetch order %s", symbol, order_ref["orderId"])

        if not filled_order:
            log.warning("%s | OCO %s ALL_DONE but no FILLED leg found", symbol, oco_id)
            continue

        exec_qty = float(filled_order["executedQty"])
        if exec_qty == 0:
            log.warning("%s | filled leg has zero executedQty", symbol)
            continue
        ...
        log.info(
            "%s | trade id=%d closed via %s — close=%.4f entry=%.4f pnl=$%.2f",
            symbol, trade["id"], exit_reason, close_price, entry, pnl,
        )
        notify.send(
